...
function createSongItem(song, queue, index) {
  ...
  const eqHTML = isActive
    ? `<div class="equalizer${isPlay ? "" : " paused"}"><span></span><span></span><span></span></div>`
  ...
  item.innerHTML = `
    ...
    <div class="song-right">
      ${eqHTML}
      <button class="song-fav-btn${isFav ? " active" : ""}" data-id="${song.id}" aria-label="Favorito">
...
function renderHome() {
  renderSongList("home-list", getAllSongs());
}
...
function backToPlaylists() { renderPlaylists(); }
...
function highlightActiveSong(songId) {
  document.querySelectorAll(".song-item").forEach(el => {
    ...
    if (eq) {
      const song = getSongById(parseInt(el.dataset.id));
      if (song) eq.outerHTML = `<span class="song-duration">${formatDuration(song.duration)}</span>`;
    }
  ...
  document.querySelectorAll(`.song-item[data-id="${songId}"]`).forEach(el => {
    ...
    if (durSpan) durSpan.outerHTML = `<div class="equalizer"><span></span><span></span><span></span></div>`;
  });
}

function highlightPlayState(playing) {
  document.querySelectorAll(".equalizer").forEach(eq => eq.classList.toggle("paused", !playing));
}
...
function updateFavButtons(songId) {
  ...
  if (Player.currentId() === songId) Player.updateFavBtn(isNowFav);
}
...
function registerSW() {
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.register("/logindemusica/sw.js", {
      scope: "/logindemusica/"
    }).then(reg => {
      console.log("SW registrado:", reg.scope);
      // Forzar activación inmediata si hay uno nuevo
